...
<section class="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
    <div class="mx-auto w-full max-w-3xl text-center print:hidden">
        ...
        <p class="mt-5 text-sm font-bold text-blue-700">خدمة الطلاب</p>
        <h1 class="mt-2 text-3xl font-black leading-10 text-slate-950 sm:text-4xl">
            استعلام الطالب عن القاعة الامتحانية
        </h1>
        <p class="mx-auto mt-3 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">
            أدخل رقمك الامتحاني لمعرفة المواد، القاعات، التاريخ، الوقت، وموقع القاعة.
        </p>
    </div>
    ...
    <form wire:submit.prevent="search" class="mx-auto w-full max-w-3xl rounded-3xl border border-white/80 bg-white/95 p-5 shadow-xl shadow-blue-950/10 transition sm:p-6 print:hidden">
        <label for="student-number" class="block text-base font-black text-slate-950">الرقم الامتحاني</label>
        <div class="mt-3 flex flex-col gap-3 sm:flex-row">
            <input
                ...
                autocomplete="off"
                maxlength="30"
                wire:model.defer="studentNumber"
            ...
            <button type="submit" class="inline-flex min-h-14 items-center justify-center rounded-2xl bg-blue-700 px-6 text-base font-black text-white shadow-lg shadow-blue-700/20 transition hover:-translate-y-0.5 hover:bg-blue-800 disabled:cursor-wait disabled:translate-y-0 disabled:opacity-75 sm:min-w-64" wire:loading.attr="disabled" wire:target="search">
                <span wire:loading.remove wire:target="search">بحث عن قاعتي الامتحانية</span>
    ...
    @if ($student)
        <section class="rounded-3xl border border-white/80 bg-white/95 p-5 shadow-lg shadow-blue-950/5 sm:p-6 print:border-slate-300 print:shadow-none">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p class="text-sm font-bold text-blue-700">بيانات الطالب</p>
                    <h2 class="mt-1 text-2xl font-black text-slate-950">{{ $student['name'] }}</h2>
                </div>
            ...
            <dl class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                    <dt class="text-xs font-bold text-slate-500">اسم الطالب</dt>
                    <dd class="mt-1 text-base font-black text-slate-950">{{ $student['name'] }}</dd>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                    <dt class="text-xs font-bold text-slate-500">الرقم الامتحاني</dt>
                    <dd class="mt-1 text-base font-black text-slate-950">{{ $student['student_number'] }}</dd>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                    <dt class="text-xs font-bold text-slate-500">الكلية</dt>
                    <dd class="mt-1 text-base font-black text-slate-950">{{ $student['college'] ?: 'غير محدد' }}</dd>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                    <dt class="text-xs font-bold text-slate-500">القسم</dt>
                    <dd class="mt-1 text-base font-black text-slate-950">{{ $student['department'] ?: 'غير محدد' }}</dd>
                </div>
            </dl>
        </section>
    @endif
            ...
            @foreach ($results as $exam)
                ...
                <article class="rounded-3xl border border-white/80 border-r-4 {{ $accentClass }} bg-white/95 p-5 shadow-lg shadow-blue-950/5 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-950/10 print:break-inside-avoid print:shadow-none">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            <p class="text-sm font-bold text-slate-500">المادة</p>
                            <h3 class="mt-1 text-2xl font-black leading-8 text-slate-950">{{ $exam['subject'] }}</h3>
                        </div>
                    ...
                    <dl class="mt-5 grid gap-3 sm:grid-cols-2">
                        <div class="rounded-2xl bg-slate-50 px-4 py-3">
                            <dt class="text-xs font-bold text-slate-500">تاريخ الامتحان</dt>
                            <dd class="mt-1 text-lg font-black text-slate-950">{{ $exam['date'] }}</dd>
                        </div>
                        <div class="rounded-2xl bg-slate-50 px-4 py-3">
                            <dt class="text-xs font-bold text-slate-500">وقت الامتحان</dt>
                            <dd class="mt-1 text-lg font-black text-slate-950">{{ $exam['time'] }}</dd>
                        </div>
                        <div class="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3">
                            <dt class="text-xs font-bold text-blue-700">القاعة</dt>
                            <dd class="mt-1 text-lg font-black text-blue-950">{{ $exam['hall'] ?: 'لم يتم تحديد القاعة بعد لهذه المادة.' }}</dd>
                        </div>
                        <div class="rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-3">
                            <dt class="text-xs font-bold text-indigo-700">موقع القاعة</dt>
                            <dd class="mt-1 text-lg font-black text-indigo-950">{{ $exam['location'] ?: 'غير محدد' }}</dd>
                        </div>
                    </dl>

                    @unless ($exam['assigned'])
                        <p class="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold leading-6 text-red-800">
                            لم يتم تحديد القاعة بعد لهذه المادة.
                        </p>
                    @endunless
                </article>
            @endforeach
