    ...
    <div dir="rtl" class="space-y-5 text-right">
                ...
                <div class="space-y-3">
                    @foreach (range(0, $period_count - 1) as $index)
                        <div class="grid gap-2 sm:grid-cols-4">
                            ...
                            <select wire:model.live="periods.{{ $index }}.period_type" class="rounded-md border-gray-300 dark:border-white/10 dark:bg-gray-800" aria-label="نوع الفترة">
                                <option value="morning">صباحية</option>
                                <option value="mid_day">وسطى</option>
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
            <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 class="text-base font-semibold text-gray-950 dark:text-white">جاهزية قوائم طلاب المواد</h2>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">يعتمد التوليد على القوائم الجاهزة فقط ضمن الكلية والعام والفصل المحددين.</p>
                </div>
                @if (($readiness['ready_rosters_count'] ?? 0) === 0)
                    <span class="rounded-full border border-warning-200 bg-warning-50 px-3 py-1 text-xs font-medium text-warning-800 dark:border-warning-500/20 dark:bg-warning-500/10 dark:text-warning-100">
                        لا توجد قائمة طلاب جاهزة لهذه المادة.
                    </span>
                @endif
            </div>
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                @foreach ([
                    'عدد القوائم الجاهزة' => $readiness['ready_rosters_count'] ?? 0,
                    'عدد الطلاب' => $readiness['students_count'] ?? 0,
                    'عدد طلاب الحملة' => $readiness['carry_count'] ?? 0,
                    'عدد المواد الأساسية' => $readiness['core_subjects_count'] ?? 0,
                    'عدد المواد المشتركة' => $readiness['shared_subjects_count'] ?? 0,
                ] as $label => $value)
                    <div class="rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-white/10 dark:bg-white/5">
            ...
            @if (($readiness['ready_rosters_count'] ?? 0) === 0)
                <div class="mt-4 rounded-md border border-warning-200 bg-warning-50 p-3 text-sm text-warning-900 dark:border-warning-500/20 dark:bg-warning-500/10 dark:text-warning-100">
                    لا توجد قائمة طلاب جاهزة لهذه المادة.
                    يجب استيراد الطلاب وتحديد القوائم كجاهزة قبل توليد البرنامج.
                </div>
            @endif
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if ($draft)
                <label class="min-w-64 space-y-1">
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-200">عند وجود برنامج رسمي لنفس المادة</span>
                    <select wire:model.live="approval_existing_strategy" class="w-full rounded-md border-gray-300 dark:border-white/10 dark:bg-gray-800">
                        <option value="create_missing">إنشاء فقط للمواد غير الموجودة</option>
                        <option value="update_existing">تحديث البرامج الموجودة</option>
                    </select>
                </label>
        ...
        @if ($draft)
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
                    ...
                    @empty
                        <div class="rounded-md border border-success-200 bg-success-50 p-3 text-sm text-success-900 dark:border-success-500/20 dark:bg-success-500/10 dark:text-success-100">
                            لا توجد مواد غير مجدولة.
                        </div>
                    @endforelse
                </div>

                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
                    <h2 class="mb-3 text-base font-semibold text-gray-950 dark:text-white">المواد الأساسية خارج الفترة الصباحية</h2>
                    @forelse ($coreWarnings as $warning)
                        <div class="rounded-md border border-warning-200 bg-warning-50 p-3 text-sm text-warning-900 dark:border-warning-500/20 dark:bg-warning-500/10 dark:text-warning-100">
                            {{ $warning['subject'] }} - {{ $warning['date'] ?? '—' }} {{ $warning['time'] ?? '' }}
                        </div>
                    @empty
                        <div class="rounded-md border border-success-200 bg-success-50 p-3 text-sm text-success-900 dark:border-success-500/20 dark:bg-success-500/10 dark:text-success-100">
                            لا توجد مواد أساسية خارج الفترة الصباحية.
                        </div>
                    @endforelse
                </div>

                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
                    <h2 class="mb-3 text-base font-semibold text-gray-950 dark:text-white">المواد المشتركة وطريقة جدولتها</h2>
                    @forelse ($sharedSummary as $shared)
                        <div class="rounded-md border border-info-200 bg-info-50 p-3 text-sm text-info-900 dark:border-info-500/20 dark:bg-info-500/10 dark:text-info-100">
                            <div class="font-medium">{{ $shared['subject'] }}</div>
                            <div class="mt-1 text-xs">{{ $shared['mode'] }}</div>
                            <div class="mt-1 text-xs">الأقسام: {{ $shared['departments_count'] }} · الأيام: {{ $shared['dates'] ?: '—' }} · الأوقات: {{ $shared['times'] ?: '—' }}</div>
                        </div>
                    @empty
                        <div class="rounded-md border border-gray-200 bg-gray-50 p-3 text-sm text-gray-600 dark:border-white/10 dark:bg-white/5 dark:text-gray-300">
                            لا توجد مواد مشتركة في هذه المسودة.
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-gray-900">
                ...
                <div class="mb-4 grid gap-3 md:grid-cols-4">
                    ...
                    <div class="flex flex-col gap-2 pt-6">
                        ...
                        <label class="flex items-center gap-2 text-sm">
                            <input type="checkbox" wire:model.live="show_core_only" class="rounded border-gray-300 text-primary-600">
                            <span>إظهار المواد الأساسية فقط</span>
                        </label>
                        <label class="flex items-center gap-2 text-sm">
                                            ...
                                            <div class="space-y-2">
                                                @forelse ($slotItems as $item)
                                                    <div class="rounded-md border p-2 shadow-sm {{ $statusClasses($item->status, $item->is_shared_subject) }}">
                                                        ...
                                                        <div class="mt-1 text-xs">الطلاب: {{ $item->student_count }} · مستجدين: {{ $item->regular_count }} · حملة: {{ $item->carry_count }}</div>
                                                        <div class="mt-1 flex flex-wrap gap-1 text-xs">
                                                            ...
                                                            @if ($item->is_core_subject)
                                                                <span class="rounded bg-white/60 px-2 py-0.5 dark:bg-black/20">مادة أساسية</span>
                                                            @endif
                                                        </div>
                                                    </div>
                                                @empty
            ...
            @if ($draft?->status === 'approved')
                <div class="rounded-lg border border-success-200 bg-success-50 p-4 text-success-900 shadow-sm dark:border-success-500/20 dark:bg-success-500/10 dark:text-success-100">
                    ...
                    <div class="mt-3 grid gap-2 md:grid-cols-4">
                        ...
                        <a href="{{ $this->invigilatorDistributionUrl() }}" class="rounded-md bg-white/70 px-3 py-2 text-sm font-medium dark:bg-black/10">الانتقال إلى توزيع المراقبين</a>
                    </div>
                </div>
            @endif
        @else
            <div class="rounded-lg border border-gray-200 bg-white p-6 text-center text-sm text-gray-500 shadow-sm dark:border-white/10 dark:bg-gray-900 dark:text-gray-400">
                لم يتم توليد مسودة بعد.
            </div>
        @endif
    </div>
</x-filament-panels::page>
